% compare_ow3d_mf12_u20_from_phi_chain.m
% Reconstruct MF12 u20 from phi20_x plus a second-order chain term and
% compare against OW3D raw/bare u20.
...
mf12 = compute_mf12_u20_from_phi_chain_local(eta11_surface, x_vec, depth_value, CFG.gravity, CFG.linear_energy_keep, four_phase_coef);

metrics_raw = compare_series_metrics_local(ow3d.u_raw, mf12.u_raw);
metrics_bare = compare_series_metrics_local(ow3d.u_bare, mf12.phix20);
metrics_chain = compare_series_metrics_local(ow3d.u_chain, mf12.chain20);

fprintf('\n=== MF12 phi+chain u20 diagnostics ===\n');
fprintf('MF12 raw(phi_x+chain) vs OW3D raw : corr = %.6f, RMSE = %.6e, peak ratio = %.6f\n', ...
    metrics_raw.corr, metrics_raw.rmse, metrics_raw.peak_ratio);
fprintf('MF12 phix20 vs OW3D bare          : corr = %.6f, RMSE = %.6e, peak ratio = %.6f\n', ...
    metrics_bare.corr, metrics_bare.rmse, metrics_bare.peak_ratio);
fprintf('MF12 chain20 vs OW3D chain        : corr = %.6f, RMSE = %.6e, peak ratio = %.6f\n', ...
    metrics_chain.corr, metrics_chain.rmse, metrics_chain.peak_ratio);
fprintf('MF12 closure residual max|u_raw-(phix20+chain20)| = %.6e\n', ...
    max(abs(mf12.u_raw - (mf12.phix20 + mf12.chain20))));

x_plot = (x_vec - 0.5 * (x_vec(1) + x_vec(end))) / CFG.lambda;
x_limits = resolve_plot_xlim_local(x_plot, eta11_surface, 5.0);
y_limits = compute_multi_series_ylimits_local({ow3d.u_raw, mf12.u_raw, ow3d.u_bare, mf12.phix20, ow3d.u_chain, mf12.chain20});

fig = create_publishable_figure_local([140 110 1500 920]);
tile = tiledlayout(fig, 3, 1, 'Padding', 'compact', 'TileSpacing', 'compact');
title(tile, sprintf('OW3D vs MF12 u_{20} reconstructed from \\phi_{20,x} + chain (\\sigma = %.3f, t index = %d, t = %.4f s)', ...
...
plot(ax1, x_plot, mf12.u_raw, '--', 'Color', [0.82 0.24 0.14], 'LineWidth', 1.8, 'DisplayName', 'MF12 (\phi_{20,x}+chain)');
hold(ax1, 'off');
...
title(ax1, 'Raw u_{20}', 'Interpreter', 'tex', 'FontSize', 13, 'FontWeight', 'normal');
legend(ax1, 'Location', 'best', 'FontSize', 10);

ax2 = nexttile(tile);
hold(ax2, 'on');
plot(ax2, x_plot, ow3d.u_bare, 'k-', 'LineWidth', 1.8, 'DisplayName', 'OW3D bare u_{20}');
plot(ax2, x_plot, mf12.phix20, '--', 'Color', [0.12 0.39 0.71], 'LineWidth', 1.8, 'DisplayName', 'MF12 \phi_{20,x}');
hold(ax2, 'off');
...
title(ax2, 'Bare / \phi_{20,x}', 'Interpreter', 'tex', 'FontSize', 13, 'FontWeight', 'normal');
legend(ax2, 'Location', 'best', 'FontSize', 10);

ax3 = nexttile(tile);
hold(ax3, 'on');
plot(ax3, x_plot, ow3d.u_chain, 'k-', 'LineWidth', 1.8, 'DisplayName', 'OW3D chain u_{20}');
plot(ax3, x_plot, mf12.chain20, '--', 'Color', [0.18 0.55 0.34], 'LineWidth', 1.8, 'DisplayName', 'MF12 chain u_{20}');
hold(ax3, 'off');
...
ylabel(ax3, '$u_{20}$ (m/s)', 'Interpreter', 'latex', 'FontSize', 13);
title(ax3, 'Chain correction only', 'Interpreter', 'tex', 'FontSize', 13, 'FontWeight', 'normal');
legend(ax3, 'Location', 'best', 'FontSize', 10);

annotation(fig, 'textbox', [0.12 0.01 0.84 0.05], ...
    'String', ['MF12 raw u_{20} is reconstructed here as \phi_{20,x} plus the subharmonic component of -\eta_x^{(1)}w^{(1)} ' ...
    'obtained from MF12 first-order fields.'], ...
    'Interpreter', 'tex', 'EdgeColor', 'none', 'HorizontalAlignment', 'left', ...
    'FontName', 'Times New Roman', 'FontSize', 11);

exportgraphics(fig, fullfile(output_dir, ...
    sprintf('compare_ow3d_mf12_u20_from_phi_chain_sigma_%03d_tidx_%04d.png', sigma_idx, selected_time_index)), ...
    'Resolution', 300);
...
save(fullfile(output_dir, sprintf('compare_ow3d_mf12_u20_from_phi_chain_sigma_%03d_tidx_%04d.mat', sigma_idx, selected_time_index)), ...
    'results', '-v7.3');

disp('MF12 phi+chain u20 comparison complete.');

function out = compute_mf12_u20_from_phi_chain_local(eta11, x_vec, depth, gravity, energy_keep, four_phase_coef)
    eta11 = eta11(:);
    x_vec = x_vec(:);
    nx = numel(x_vec);
    dx = x_vec(2) - x_vec(1);
    ...
    [eta20, phi20, ~, ~, w20] = mf12_second_subharmonic_kinematics(coeffs2, x_vec.', 0, 0, 0);
    phix20 = spectral_derivative_x_local(phi20(:), x_vec);
    ...
    out.w20 = w20(:);
    out.chain20 = chain20(:);
    out.u_raw = out.phix20 + out.chain20;
