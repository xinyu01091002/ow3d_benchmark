% compare_ow3d_mf12_u20_raw.m
% Build an MF12-style raw u20 by adding a chain-correction term to the
% MF12 bare difference-wave velocity, and compare against OW3D raw u20.
...
mf12 = compute_mf12_raw_subharmonic_surface_local( ...
    eta11_surface, x_vec, depth_value, CFG.gravity, CFG.linear_energy_keep, four_phase_coef);

metrics_raw = compare_series_metrics_local(ow3d.u_raw, mf12.u_raw);
metrics_bare = compare_series_metrics_local(ow3d.u_bare, mf12.u_bare);
metrics_chain = compare_series_metrics_local(ow3d.u_chain, mf12.u_chain);

fprintf('\n=== MF12 raw u20 diagnostics ===\n');
fprintf('MF12 raw vs OW3D raw  : corr = %.6f, RMSE = %.6e, peak ratio = %.6f\n', ...
    metrics_raw.corr, metrics_raw.rmse, metrics_raw.peak_ratio);
fprintf('MF12 bare vs OW3D bare: corr = %.6f, RMSE = %.6e, peak ratio = %.6f\n', ...
    metrics_bare.corr, metrics_bare.rmse, metrics_bare.peak_ratio);
fprintf('MF12 chain vs OW3D chain: corr = %.6f, RMSE = %.6e, peak ratio = %.6f\n', ...
    metrics_chain.corr, metrics_chain.rmse, metrics_chain.peak_ratio);
fprintf('MF12 closure residual max|u_raw-(u_bare+u_chain)| = %.6e\n', ...
    max(abs(mf12.u_raw - (mf12.u_bare + mf12.u_chain))));

x_plot = (x_vec - 0.5 * (x_vec(1) + x_vec(end))) / CFG.lambda;
x_limits = resolve_plot_xlim_local(x_plot, eta11_surface, 5.0);
y_limits_main = compute_multi_series_ylimits_local({ow3d.u_raw, mf12.u_raw, ow3d.u_bare, mf12.u_bare});
y_limits_comp = compute_multi_series_ylimits_local({ow3d.u_chain, mf12.u_chain, ow3d.u_raw, mf12.u_raw});

fig = create_publishable_figure_local([140 110 1500 920]);
tile = tiledlayout(fig, 3, 1, 'Padding', 'compact', 'TileSpacing', 'compact');
title(tile, sprintf('OW3D vs MF12 raw u_{20} comparison (\\sigma = %.3f, t index = %d, t = %.4f s)', ...
    sigma_value, selected_time_index, t_selected), ...
    'Interpreter', 'tex', 'FontSize', 16, 'FontWeight', 'bold');

ax1 = nexttile(tile);
hold(ax1, 'on');
plot(ax1, x_plot, ow3d.u_raw, 'k-', 'LineWidth', 1.8, 'DisplayName', 'OW3D raw u_{20}');
plot(ax1, x_plot, mf12.u_raw, '--', 'Color', [0.82 0.24 0.14], 'LineWidth', 1.8, 'DisplayName', 'MF12 raw u_{20}');
plot(ax1, x_plot, ow3d.u_bare, '-', 'Color', [0.18 0.55 0.34], 'LineWidth', 1.4, 'DisplayName', 'OW3D bare u_{20}');
plot(ax1, x_plot, mf12.u_bare, ':', 'Color', [0.12 0.39 0.71], 'LineWidth', 1.8, 'DisplayName', 'MF12 bare u_{20}');
hold(ax1, 'off');
...
title(ax1, 'Raw and bare u_{20}', 'Interpreter', 'tex', 'FontSize', 13, 'FontWeight', 'normal');
legend(ax1, 'Location', 'best', 'FontSize', 10);
text(ax1, 0.02, 0.95, sprintf('raw corr = %.3f, RMSE = %.2e\nbare corr = %.3f, RMSE = %.2e', ...
    metrics_raw.corr, metrics_raw.rmse, metrics_bare.corr, metrics_bare.rmse), ...
    'Units', 'normalized', 'VerticalAlignment', 'top', 'BackgroundColor', 'w', 'Margin', 2, 'FontSize', 10);

ax2 = nexttile(tile);
hold(ax2, 'on');
plot(ax2, x_plot, ow3d.u_chain, 'k-', 'LineWidth', 1.8, 'DisplayName', 'OW3D chain u_{20}');
plot(ax2, x_plot, mf12.u_chain, '--', 'Color', [0.82 0.24 0.14], 'LineWidth', 1.8, 'DisplayName', 'MF12 chain u_{20}');
hold(ax2, 'off');
grid(ax2, 'on'); box(ax2, 'on');
set(ax2, 'FontName', 'Times New Roman', 'FontSize', 12, 'LineWidth', 1.0);
xlim(ax2, x_limits);
ylim(ax2, y_limits_comp);
xlabel(ax2, '$x / \lambda$', 'Interpreter', 'latex', 'FontSize', 13);
ylabel(ax2, '$u_{20}$ (m/s)', 'Interpreter', 'latex', 'FontSize', 13);
title(ax2, 'Chain correction only', 'Interpreter', 'tex', 'FontSize', 13, 'FontWeight', 'normal');
legend(ax2, 'Location', 'best', 'FontSize', 10);

ax3 = nexttile(tile);
hold(ax3, 'on');
plot(ax3, x_plot, ow3d.u_raw - mf12.u_raw, 'Color', [0.12 0.39 0.71], 'LineWidth', 1.8, 'DisplayName', 'raw diff');
plot(ax3, x_plot, ow3d.u_chain - mf12.u_chain, 'Color', [0.82 0.24 0.14], 'LineWidth', 1.8, 'DisplayName', 'chain diff');
plot(ax3, x_plot, ow3d.u_bare - mf12.u_bare, 'Color', [0.18 0.55 0.34], 'LineWidth', 1.8, 'DisplayName', 'bare diff');
hold(ax3, 'off');
grid(ax3, 'on'); box(ax3, 'on');
set(ax3, 'FontName', 'Times New Roman', 'FontSize', 12, 'LineWidth', 1.0);
xlim(ax3, x_limits);
xlabel(ax3, '$x / \lambda$', 'Interpreter', 'latex', 'FontSize', 13);
ylabel(ax3, 'Difference', 'Interpreter', 'tex', 'FontSize', 13);
title(ax3, 'OW3D - MF12 differences', 'Interpreter', 'tex', 'FontSize', 13, 'FontWeight', 'normal');
legend(ax3, 'Location', 'best', 'FontSize', 10);

annotation(fig, 'textbox', [0.12 0.01 0.84 0.05], ...
    'String', ['MF12 raw u_{20} is built as MF12 bare difference-wave u_{20} plus a chain term extracted from the ' ...
    'four-phase subharmonic of -\eta_x^{(1)} w^{(1)} using MF12 first-order fields.'], ...
    'Interpreter', 'tex', 'EdgeColor', 'none', 'HorizontalAlignment', 'left', ...
    'FontName', 'Times New Roman', 'FontSize', 11);

exportgraphics(fig, fullfile(output_dir, ...
    sprintf('compare_ow3d_mf12_u20_raw_sigma_%03d_tidx_%04d.png', sigma_idx, selected_time_index)), ...
    'Resolution', 300);
...
results.ow3d = ow3d;
results.mf12 = mf12;
results.metrics_raw = metrics_raw;
results.metrics_bare = metrics_bare;
results.metrics_chain = metrics_chain;
save(fullfile(output_dir, sprintf('compare_ow3d_mf12_u20_raw_sigma_%03d_tidx_%04d.mat', sigma_idx, selected_time_index)), ...
    'results', '-v7.3');

disp('MF12 raw u20 comparison complete.');

function out = compute_mf12_raw_subharmonic_surface_local(eta11, x_vec, depth, gravity, energy_keep, four_phase_coef)
    eta11 = eta11(:);
    x_vec = x_vec(:);
    nx = numel(x_vec);
    dx = x_vec(2) - x_vec(1);
    ...
    coeffs1 = mf12_direct_coefficients(1, gravity, depth, a, b, kx, ky, 0, 0, 0);
    coeffs2 = mf12_direct_coefficients(2, gravity, depth, a, b, kx, ky, 0, 0, 0);
    [eta20, phi20, u20_bare, ~, w20] = mf12_second_subharmonic_kinematics(coeffs2, x_vec.', 0, 0, 0);

    [u1, ~, w1] = kinematicsMF12(1, coeffs1, x_vec.', 0, 0, 0); %#ok<ASGLU>
    eta1 = reconstruct_eta1_local(coeffs1, x_vec);
    etax1 = spectral_derivative_x_local(eta1, x_vec);

    eta_x_phase = generate_phase_shifted_fields_local(etax1, 0:90:270);
    w1_phase = generate_phase_shifted_fields_local(w1(:), 0:90:270);
    chain_phase = -eta_x_phase .* w1_phase;
    chain_harmonics = reconstruct_harmonics_1d_local(chain_phase, four_phase_coef);
    u20_chain = chain_harmonics(4, :).';
    ...
    out.w = w20(:);
    out.u_bare = u20_bare(:);
    out.u_chain = u20_chain(:);
    out.u_raw = out.u_bare + out.u_chain;
...
function eta1 = reconstruct_eta1_local(coeffs1, x_vec)
    theta = coeffs1.omega(:) .* 0 - coeffs1.kx(:) .* x_vec(:).' - coeffs1.ky(:) .* 0;
    eta1 = sum(coeffs1.a(:) .* cos(theta) + coeffs1.b(:) .* sin(theta), 1).';
end

function phase_fields = generate_phase_shifted_fields_local(field1, phase_deg)
    field1 = field1(:);
    analytic_field = hilbert(field1);
    phase_fields = zeros(numel(phase_deg), numel(field1));
    for idx = 1:numel(phase_deg)
        phase_fields(idx, :) = real(analytic_field .* exp(1i * deg2rad(phase_deg(idx))));
    end
end
...
function keep_idx = select_energy_dominant_indices_local(fft_eta, candidate_idx, energy_keep)
    ...
    keep_idx = sort(candidate_idx(order(1:cutoff_pos)));
end
...
function x_limits = resolve_plot_xlim_local(x_plot, eta11, plot_window_lambda)
    eta11 = eta11(:);
    x_plot = x_plot(:);
